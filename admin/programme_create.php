...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $image = $_POST['image'];
    $stmt = $pdo->prepare("INSERT INTO Programmes (ProgrammeName, LevelID, ProgrammeLeaderID, Description, Image) VALUES (?, ?, ?, ?, ?)");
    $stmt->execute([$name, $level, $leader, $desc, $image]);
...
<div class="form-container">
    <h1><i class="fas fa-plus-circle"></i> Add New Programme</h1>
    <form method="post" class="admin-form">
        <div class="form-group">
            <label for="name"><i class="fas fa-tag"></i> Programme Name</label>
            <input type="text" name="name" id="name" required placeholder="e.g., BSc Computer Science">
        </div>
        ...
        <div class="form-group">
            ...
            <select name="level" id="level">
                <?php foreach ($levels as $l): ?>
                    <option value="<?= $l['LevelID'] ?>"><?= htmlspecialchars($l['LevelName']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="description" id="description" rows="6" placeholder="Enter programme description..."></textarea>
        </div>
        ...
        <div class="form-buttons">
            <button type="submit" class="btn-save"><i class="fas fa-save"></i> Save Programme</button>
