...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!file_exists($staffFile)) die('Staff configuration missing.');
    $staffData = json_decode(file_get_contents($staffFile), true);
    $authenticated = false;
    $staffInfo = null;
    foreach ($staffData['staff'] as $staff) {
        if ($staff['username'] === $username && password_verify($password, $staff['password'])) {
            $authenticated = true;
            $staffInfo = $staff;
            break;
        }
    ...
    if ($authenticated) {
        ...
        $_SESSION['staff_id'] = $staffInfo['staff_id'];
        $_SESSION['staff_name'] = $staffInfo['name'];
